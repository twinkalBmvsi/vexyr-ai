    ...
    if (subscription?.modules?.extraBots) {
      ...
      if (eb.assigned_slots !== undefined || eb.unassigned_slots !== undefined) {
        ...
        activeAssigned.forEach(([id, slot]: [string, any]) => {
          expirationMap[id] = new Date(slot.expires_at).toLocaleDateString("en-US", { year: "numeric", month: "short", day: "numeric" })
        })
        const activeUnassigned = (eb.unassigned_slots || []).filter((slot: any) => slot.expires_at && new Date(slot.expires_at) > new Date()).length
        extraBots = activeAssigned.length + activeUnassigned
      } else {
        // Legacy fallback
        if (Array.isArray(eb)) {
          eb.filter((bot: any) => bot.expires_at && new Date(bot.expires_at) > new Date()).forEach((bot: any) => {
      ...
      >
        <div>
          ...
          <p className="dash-subtitle">
            Manage your automated conversational agents.
          </p>
        </div>

        {canCreateAgent ? (
          <Link
            href={`/${resolvedParams.tenantSlug}/agents/new`}
      ...
      {!canCreateAgent && agents.length > 0 && (
        ...
        >
          ...
          <span>
            ...
            <strong>
              {agentLimit} AI {agentLimit === 1 ? "agent" : "agents"}
            </strong>
            {extraBots > 0
      ...
      <div className="dash-grid">
        {agents.length === 0 ? (
          ...
          >
            ...
            >
              You haven't created any AI agents yet. Build your first
              intelligent assistant to start automating your scheduling and
          ...
          agents.map((agent, index) => {
            const isBase = index < BASE_AGENT_LIMIT;
            const exp = isBase
              ? "Included in Plan"
              : expirationMap[agent.id] || "Expired";
            ...
            return (
              <div key={agent.id} className="dash-card">
                ...
                <div style={{ marginTop: "auto", paddingTop: "2rem" }}>
                  <Link
                    href={`/${resolvedParams.tenantSlug}/agents/${agent.id}`}
