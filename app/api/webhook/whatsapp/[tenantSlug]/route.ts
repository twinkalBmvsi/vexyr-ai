  ...
  try {
    ...
    if (!message) {
      return NextResponse.json(
        { status: "ignored", reason: "No message content" },
        { status: 200 },
      );
    }
    ...
    if (!text) {
      return NextResponse.json(
        { status: "ignored", reason: "No text body" },
        { status: 200 },
      );
    }
    ...
    if (!channel) {
      return NextResponse.json(
        { status: "ignored", reason: "WhatsApp channel not configured" },
        { status: 200 },
      );
    }
    ...
    if (channel.is_active === false) {
      return NextResponse.json(
        { status: "ignored", reason: "WhatsApp channel deactivated" },
        { status: 200 },
      );
    }
      ...
      if (waToken && phoneId) {
        await fetch(`https://graph.facebook.com/v18.0/${phoneId}/messages`, {
          ...
          body: JSON.stringify({
            ...
            type: "text",
            text: {
              body: "Our AI assistant is currently unavailable due to high volume. Please contact the business directly.",
            },
          }),
        });
      }
      return NextResponse.json(
        { status: "ignored", reason: "Rate limit reached" },
        { status: 200 },
      );
    }
    ...
    if (!agent) {
      return NextResponse.json(
        { status: "ignored", reason: "No agent found" },
        { status: 200 },
      );
    }
    ...
    if (agent.business_rules) {
      try {
        ...
        if (
          ...
          !rules.active_channels.includes("whatsapp")
        ) {
          return NextResponse.json(
            {
              status: "ignored",
              reason: "WhatsApp is not active for this agent",
            },
            { status: 200 },
          );
        }
    ...
    const senderName =
      value?.contacts?.[0]?.profile?.name || `WhatsApp User (${fromNumber})`;
    ...
    const { data: matches } = await supabaseAdmin
      ...
      .eq("tenant_id", tenant.id)
      .eq("channel", "whatsapp")
      .or(`phone.eq.${fromNumber},metadata->>whatsapp_id.eq.${fromNumber}`)
      .order("created_at", { ascending: false })
      .limit(1);
    ...
    if (exactMatch) {
      ...
      const { data: newCustomer } = await supabaseAdmin
        ...
        .insert({
          ...
          phone: fromNumber,
          channel: "whatsapp",
          metadata: { whatsapp_id: fromNumber },
        })
        .select("*")
        .single();
      ...
      if (
        ...
        for (const toolCall of responseMessage.tool_calls) {
          ...
          if (fnName === "book_appointment") {
            console.log("AI calling book_appointment with args:", fnArgs);
            ...
            console.log("book_appointment output:", toolOutput);
          } else if (fnName === "reschedule_appointment") {
            console.log("AI calling reschedule_appointment with args:", fnArgs);
            ...
            console.log("reschedule_appointment output:", toolOutput);
